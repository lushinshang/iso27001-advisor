...
> 狀態：✅ **v4.2 全部完成**（收尾記錄見 §11）。237 tests passed（含雙資料集護欄實跑）。後續：v4.3 入口接線
...
### 11.1 收尾執行結果（§10.7 清單全數完成）

| 任務 | 結果 | commit |
|---|---|---|
| gated 設為預設 | `HybridSearcher.search()` 預設 gated；既有測試改顯式 `mode="full"` | 8c46007 |
| 雙資料集回歸護欄 | `tests/test_gated_recall_regression.py`：探測式 skip（實測 Ollama 可達性再決定，不吞真 bug） | 9bd1fef |
| 護欄捨入修復 | 見 §11.2 | 5449a60 |
| README 更新 | 雙軌指標表、過期數字作廢、目錄實檔化、LV5 部分達成 | 73fbe70 |
| spec HTML 全面同步 | system_spec.html → Spec v1.6、architecture.html 加閘門節點、兩份歷史計畫加歸檔橫幅 | 54bf9dc |

**最終驗證：237 tests passed, 0 skipped（護欄於 Ollama 可達環境實跑通過）**；
README 文件指令實跑重現表格數字（gated 改寫題 76.7 / 58.1 / 24.6 / 0.6222）。

### 11.2 收尾階段的最後一個 bug：護欄捨入邊界

Codex sandbox 擋 localhost，護欄測試在其環境只能 skip、從未跑綠。真環境首跑
即紅：改寫題實測 46/60 = 76.666...%，門檻卻寫成報表四捨五入後的 76.7%——
`76.666 ≥ 76.7` 永久失敗，護欄天生是壞的。修法：百分比比較改為**整數命中數
比較**（`hit_count >= 46`、`>= 59`），捨入問題從根本消除。

**教訓**：(1) sandbox 中 skip 的測試不算驗證過，護欄類測試必須在依賴可達的
環境至少跑綠一次；(2) 邊界值永遠不用「顯示用的捨入值」——報表寫 76.7% 是
給人看的，程式比較要用真實值（46/60）。

### 11.3 全程回顧：八次轉折的決策軌跡

```
v1.0 規劃：五道關卡、全地端、預註冊規則
 → 煙霧測試抓到 parser 冤案（gemma4 fenced JSON，0%→100%）
 → 正式評測觸發誠實率紅線（12.73%）→ LLM 產線封存、pdca 改道（v1.2）
 → 期末考退步 3.3% → G5 一票否決擋下（v1.3）
 → 消融實驗分離兩個退步根因（RRF 稀釋 / KG 插隊）
 → 信心閘門 + 先驗預測（未中 → 揪出任務 A 報告的錯誤數字 q52=88.6 非 180）
 → 改寫題集實測過擬合 -28.3pp → hybrid 價值定量證實（+5.0pp）
 → 擁有者記錄性取捨 → 合併（v1.4）→ 收尾（§11.1）
```

每個轉折都由數據觸發、每次標準修訂都有白紙黑字。本文件連同 git 歷史
（f292082 起）構成完整的 Evaluation Flywheel 實踐記錄。

### 11.4 遺留事項

- **v4.3**：`app.py` / `main.py` / `agent.py` 接線至 HybridSearcher（gated）
- **LLM 提名產線重啟**（選配）：gemma4:12b-it-qat-16k + 句子編號選擇法（§9.3），
  目標為 pdca 未覆蓋的 51 節點
- **改寫考場天花板 76.7%**：gated 仍有 14 題失敗（12 題與 keyword 共同失敗），
  候選方向：同義詞擴充、KG 補邊、embedding 模型升級

---

## 12. v4.4：MCQ 多選支援與選項分解檢索（2026-07-04 定案）

### 12.1 背景：真實使用者回報的兩個疊加 bug

使用者以 ISO 27001:2022 附錄 A 實體控制措施多選題（正解 A/C/D）實測，
系統只回答 A，且**編造 C/D 不屬於實體控制的錯誤理由**。診斷（實測數據）：

1. **MCQ 指令硬編碼單選**：`llm.py` `_is_mcq()` 偵測到選擇題後注入
   「建議答案：X（最正確的選項字母）」——不辨識「哪些」等多選訊號，
   多選題被降維成單選。System Prompt 規則 3b 同病。
2. **干擾項污染檢索**：整題（含干擾項）檢索時，選項 B「遠端工作」觸發
   intent boost +180 令 `control_6.7` 霸榜（316 分），正解 `control_7.2`/
   `control_7.13` 完全未進 context（0/2）；**各選項單獨檢索**則
   (A)→7.2（153）、(C)→7.13（182）精準命中，(D) 最高 83 分 < 100
   （gated 模式下會自動交給向量分支）。
   根因：MCQ 干擾項本為誤導而設計，整題檢索等於讓陷阱污染 context。

### 12.2 規格

**(a) 多選偵測**：`_mcq_mode(query) -> None | "single" | "multi"`
- 沿用既有 `_is_mcq` 的選項格式偵測為前置條件（無選項一律 None）
- 有選項且題幹含多選訊號（「哪些」「複選」「多選」「所有正確」等）→ `"multi"`
- 有選項無多選訊號 → `"single"`（既有行為完全不變，含既有 eval 題）

**(b) 多選指令**：mode 為 multi 時注入
「✅ 建議答案：X、Y（列出**所有**正確選項）」＋逐項附條文依據＋
說明其餘選項不屬於的理由；System Prompt 規則 3b 同步改寫涵蓋單/多選。

**(c) 選項分解檢索**：`core` 新增純函數與檢索協調器
- `split_mcq_options(query) -> {"stem": str, "options": {"A": str, ...}}`
  （支援 `(A)`、`A.`、`A）` 等格式）
- `mcq_union_search(searcher, query, limit=4, per_option=2) -> list`
  題幹檢索 top-limit ＋ 每選項檢索 top-per_option，聯集去重
  （同 id 取最高分），依分數重排。searcher 只需具備 `.search()` 介面
  （ISO27001Searcher 與 HybridSearcher 皆可用）
- 接線：`main.py` 與 `app.py` 的檢索呼叫點，偵測到 MCQ 時改走
  `mcq_union_search`（最小 diff，不動其他邏輯）

**(d) 回歸保護**：本回報題目寫入單元測試——斷言
`mcq_union_search` 的結果聯集包含 `control_7.2` 與 `control_7.13`。
既有 60 題評估集無多選題，雙資料集護欄不受影響（不改動評估集）。

### 12.3 驗收標準

1. TDD：偵測（single/multi/None 各 ≥ 2 案例）、選項拆解（三種格式）、
   聯集檢索（真實回報題命中 7.2＋7.13）測試先紅後綠
2. 全套 `python3 -m pytest -q` 綠（既有 237＋新增，0 failed）
3. 既有單選 MCQ 行為不變（faithfulness 評估集含單選題，指令輸出格式不變）
4. e2e 抽查：以回報原題跑 `main.py`，引用區出現 7.x 實體控制條文、
   建議答案為多選格式（此項需 Ollama，人工確認）

### 12.4 附帶收穫

真實使用者是最好的評估集擴充來源——評估集無多選題，故此缺陷從未被
任何指標照到。後續（另案）：建立 MCQ 專屬評估集。
